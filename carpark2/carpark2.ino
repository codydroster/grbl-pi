...
// Hold the scanner off until the carried bin is within this of the carpark.
// Fired at the start of the run home it is decoding while the bin is still
// deep in the lane, where the nearest thing in front of it may be a bin in
// another slot entirely - close in, the bin on the forks is the only candidate.
// The car is coming OUT, so the reading falls as it travels; the wait is timed
// from the calibrated speed rather than polled, because a rangefinder read
// takes ~670ms and the drive home is only a couple of seconds.
const int      SCAN_ARM_MM  = 300;
...
// scanAfterMs > 0 fires the scanner that far into the run, once the bin is
// close enough to be the only thing the scanner can see (see SCAN_ARM_MM).
// Timed, not measured: the home run is short and a rangefinder read would eat
// most of it. 0 means do not scan at all.
bool driveToHome(uint32_t scanAfterMs) {
  carSpeed('R', DRIVE_SPEED);
  bool crept = false;
  uint32_t t0 = millis();
  while (millis() - t0 < DRIVE_TIMEOUT_MS) {
    pump();
    if (scanAfterMs && !scanArmedAt && millis() - t0 >= scanAfterMs) {
      lastBarcode[0] = 0;
      scannerCmd("SCAN");
      scanArmedAt = millis();
    }
...
void retrieve() {
  ...
  liftUp();
  if (!driveToHome(0)) { say("RETRIEVE FAIL not home"); return; }
  say("RETRIEVE DONE");
}
...
void handleCommand(Stream &port) {
  ...
  else if (c == 'g') say(driveToHome(0) ? "HOME DONE" : "HOME FAIL");
  // --- reports ---
  else if (c == 'h') {
    ...
    // is free. What it must not do is start decoding while the bin is still
    // deep in the lane, so the trigger waits until the bin is inside
    // SCAN_ARM_MM. One stationary reading before setting off says how far that
    // is, and the calibrated speed turns it into a delay.
    lastBarcode[0] = 0;
    scanArmedAt = 0;
    int d0 = readDistanceMM();
    uint32_t scanAfterMs = 1;           // already close (or no reading): at once
    if (d0 > SCAN_ARM_MM) {
      scanAfterMs = (uint32_t)(d0 - SCAN_ARM_MM + DEPTH_RAMP_MM) * 1000
                    / DRIVE_MM_PER_S;
      Serial1.print("scanning once within "); Serial1.print(SCAN_ARM_MM);
      Serial1.print("mm, in "); Serial1.print(scanAfterMs); Serial1.println("ms");
      Serial.print("scanning once within "); Serial.print(SCAN_ARM_MM);
      Serial.print("mm, in "); Serial.print(scanAfterMs); Serial.println("ms");
    }
    bool ok = driveToHome(scanAfterMs);
